...
async function Ready()
{
    ...
    function DDM_to_UI_variables()
    {
        ...
        for (const parentKey in UI)
        {
            ...
            {
                let child = parentObj[childKey];
                const sessionValue = child.sessionValue;
                const domEl = document.getElementById(childKey); // ❗❗❗
                if (domEl)
                {
                    parentObj[childKey] = child = domEl;
                ...
                {
                    if (child.hasOwnProperty('baseValue'))
                    {
                        parentObj[childKey] = child.sessionValue || child.baseValue;
                    }
                    else if (child.baseKey?.hasOwnProperty('baseValue'))
                    {
                        parentObj[childKey] = child.baseKey.sessionValue || child.baseKey.baseValue;
                    }
                    else
                    {
                        if (childKey == 'baseKey' || child.inputType == "prompt" || child.baseKey?.inputType == "prompt")
                        {
    ...
    function SaveSettingsOnChanges()
    {
        for (const parentKey in UI)
        {
            ...
            for (const childKey in parentObj)
            {
                ...
                if (parentKey == 'range') // special case...
                {
